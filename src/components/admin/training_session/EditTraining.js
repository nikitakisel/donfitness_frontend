...
function EditTraining({ token, trainingId, setActiveTab }) {
  ...
  const handleEditTrainingSession = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(
        `${API_BASE_URL}/training_sessions/${trainingId}`,
        {
      ...
      setActiveTab("infoTrainings");
    } catch (error) {
      console.error("Edit training session failed:", error);
      alert("Edit training session failed. Check the console for details.");
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchTrainingInfo = async () => {
      try {
        ...
        setTrainingTypeId(data.training_type_id);
        ...
        alert("Failed to fetch all training. Check the console for details.");
      }
    };
  ...
  return (
    <div>
      <h2 className="Chapter-title">Изменить тренировку</h2>
      <div className="Auth-form-container">
        <form className="Auth-form" onSubmit={handleEditTrainingSession}>
