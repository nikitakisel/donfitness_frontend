...
function MyProfile({ token }) {
  ...
  const handleProfileChanges = async (e) => {
    e.preventDefault();
    try {
      const getResidentIdResponse = await axios.get(
      ...
      const changeProfileDataResponse = await axios.put(
  ...
  useEffect(() => {
    const fetchResident = async () => {
      try {
        ...
        const getResidentInfoResponse = await axios.get(
          `${API_BASE_URL}/residents/${residentId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  return (
    <div>
      <h2 className="Chapter-title">Мой профиль</h2>
